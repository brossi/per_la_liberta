...
| S1.3a.1 | **`build_canonical` fails loud on >2 structural witnesses** — it uses `witness_order[0:2]` and currently **silently ignores** `[2:]`; raise `CaptureError` ("N-way structural alignment unbuilt") so "exactly two structural witnesses" is a documented boundary, not a silent truncation (#16 audit, Thread 2) | §11.1; #16 | S1.3a | negative: ≥3 witnesses raises `CaptureError`; the 2-witness path unchanged | `BUILD` | `TODO` |
| S1.3a.2 | **Make `align_streams`'s `SequenceMatcher` junk policy explicit** — pass `autojunk` rather than rely on the implicit default; comment that it is load-bearing (PLL canonical count swings 4786↔5226, ~9%, and the discarded keys are real OCR noise — `3e`/`s:`/`35`) and mirrors `reconcile.align_paragraphs` (#16 audit, Thread 3B) | `reconcile.py:188`; #16 | S1.3a | property: junk policy explicit + documented; canonical count unchanged at the chosen setting | `BUILD` | `TODO` |
| S1.3a.3 | **Oracle-back the five frozen real-input counts** (278/521/3621/3356/4786) — assert each against an **independent** re-computation in-test (non-blank-line runs for body, regex for furniture, alignment-opcode algebra for canonical) instead of a written-down int, so a segmentation regression **reds** instead of being pinned (#16 audit, Thread 3A) | §9; #16 | S1.3a | property: each count == its independent oracle; mutating the segmenter reds the binding | `SCAF` (test) | `TODO` |
| S1.3a.4 | **Canonical-page tripwire** — pin that every canonical atom is `PAGE_UNMAPPED` today (canonical adopts copy1's unmapped address; copy3, the only page-bearing witness, is word-level → absent from the structural alignment), so S7.1b's page-attribution change is **loud + intentional**; this test is S7.1b's worklist marker (#16 audit, Thread 1) | §11.1; D25; #16 | S1.3a | property: canonical stream is uniformly `PAGE_UNMAPPED`; assertion referenced by S7.1b | `SCAF` (test) | `TODO` |
| S1.3a.5 | **Honest-prose fixes** in issue #16 / this tracker / plan §11.1 — "45% both-derived" is a **structural (positional)** pairing stat, **not** content agreement (only 276/4786 are content-anchored `equal`); name `key=` as the per-book alignment lever; state the exactly-two-structural-witness boundary; state canonical page-attribution → S7.1b (#16 audit, Thread 3C/2/1) | #16 | S1.3a | the four claims corrected wherever they appear (issue body/comment, tracker, plan §11.1) | `SCAF` (docs) | `TODO` |
| S1.3b | **Typed projection over the raw atoms** via the `BlockClassifier` (S0.4 stub → S9 real): each typed atom carries `typed_by` + `confidence` that **B may override** (re-type, R3/D5). **Profile-scoped completeness check:** unknowns in the profile's declared `boundary_classes` (heading / footnote-call / embedded-letter boundary — S9.1) **hard-fail**; unknowns in body leaves **route to review** with count + location (reuses the S6 truth-table machinery). An **all-`unknown` projection FAILS** completeness — `unknown` is a first-class *incomplete* state, never a degenerate green | §2-A; D5/R3 | S1.3a, **S0.4** | property: boundary-class unknown raises; body-leaf unknown routes-to-review with count+location; all-`unknown` fails completeness | `BUILD` | `TODO` |
...
>   not the *limitation*.
> - **`autojunk=True` is made explicit, not validated.** S1.3a.2 ends the reliance on the *silent
>   default*; it does not prove `True` is the right policy. The junk heuristic discards
>   high-frequency keys — good on PLL's OCR-noise copies (the discarded keys are noise), but
>   possibly wrong on a cleaner witness set where a legitimately-repeated key (a refrain, a
>   formula) would be junked and an anchor lost. **Return to review the policy itself** (keep
>   `True` / set `False` / inject a per-book junk predicate) when a second book's witnesses are
>   aligned, or if a canonical anchor count looks degraded.
