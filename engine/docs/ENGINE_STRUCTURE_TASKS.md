...
| | **S2.0 outcome (POSITIVE, conditional).** Probe `books/per_la_liberta/probes/s2_0_geometry_probe.py`; written result `docs/probes/s2_0_geometry_alignment.md`. Both PDFs are image scans (0 native boxes); a valid box layer is **generated** by fresh PyMuPDF+Tesseract(ita,300dpi) — in-bounds 0.999. Cross-engine token anchorability vs copy3/Gemini: BoW median **0.939** book-wide (n=37, min 0.788, 34/37 ≥0.85). Body is **uniformly two-column (37/37)** → naive full-width ordered coverage collapses to **0.498**, but recovering column reading-order **from the box coords** lifts it to **0.928** (clears the pre-set 0.85 primary bar). **Decision (a): S2.1 NORMAL path** (not S2.1-alt) + matcher must be **column-aware** & **confidence/density-gated** (near-blank pages hallucinate boxes; single-col pages must not be force-split). **Decision (b): S5 geometry = PRIMARY, per-atom confidence-gated** (not demoted); absence first-class, coords never invented | | #18 | written probe result delivered; both decisions evidence-chosen | `GATE` | `DONE` |
| S2.1 | `GeometrySource` seam (Protocol) + backend on the **path S2.0 selected** (= NORMAL, fresh PyMuPDF+Tesseract(ita) OCR — both PDFs are image scans, no native layer): textpage/bbox layer → `atom.geom` word-box union, via an **explicit witness-text↔geometry matcher with its own fail-loud mode**; the matcher writes `{geometry_engine, match_method, match_confidence}` into S1.1's frozen slot; **unmatched boxes are unusable for primary re-bind**; canonical atom carries its primary witness's box **only where matched**. **S2.0-mandated segmentation front-end** (design inputs in `docs/probes/s2_0_geometry_alignment.md` §"S2.1 design inputs"): **density pre-check** (fixed book-calibrated ink-fraction, NOT per-page Otsu which inflates on blanks → near-blank pages emit 658 hallucinated boxes → `geom.present=false`); **column / reading-order detection** via vertical projection profile / Recursive X-Y Cut (gutter valley is the discriminator, mirror-symmetry corroborates, cross-page prior; naive full-width order scores ~0.50 on PLL's uniformly two-column body, ~0.93 column-aware) — RXYC yields reading order for free; **human-review worklist** for any page the front-end can't score confidently (page image + tentative split/boxes overlaid) **before it passes the gate** — classifiers calibrate to **abstain**, never silently guess (per [[project-ingestion-human-in-loop]], S5.2 monotone-strictness). `geom.present=false` only after BOTH auto-detection and human review decline | §3.0, §11.1; D30; `docs/probes/s2_0_geometry_alignment.md` | S1.1, S2.0 | seam injectable; backend yields matched boxes (+ provenance) for a PLL page fixture; unmatched → `geom.present=false`, not invented; low-confidence pages routed to human review | `BUILD` | `TODO` |
| S2.1-alt | **Negative-branch fallback as a real deliverable** (only built if S2.0 is negative/below-threshold): name the coordinate source (word-coordinate re-derivation, or accept no-geometry), the atom-matching method, the acceptable quality threshold for S3/S5, and **what is disabled if unmet** (S3's geometry base layer; geometry as S5's *primary* anchor → S5 runs geometry-demoted / no-geometry). Not an architectural pause — a scoped path. **S2.0 outcome: NOT triggered** (probe positive — geometry usable as S5 primary); spec retained as the conditional fallback | §3.0; D30 | S2.0 (negative) | the fallback path is specified + (if triggered) implemented; S3/S5 know exactly what they lose | `BUILD` (conditional) | `N/A (S2.0 positive)` |
...
| ID | Deliverable | Refs | Deps | Done-when (tier) | Tag | St |
|---|---|---|---|---|---|---|
| S5.1 | `rebind_anchors` per node: **geometry region** (primary where matched, from D30 word-boxes), **fuzzy/fail-loud content fingerprint** (never the exact-substring the live tree tombstoned), **structural-path** tie-break. **Three explicit operating modes — `geometry-primary` \| `geometry-tie-break` \| `no-geometry` — selected by S2.0's outcome and recorded in the structure-map lineage / rebind config** (M2), so a re-bind result is interpretable after the fact. **S2.0 selected `geometry-primary`, per-atom confidence-gated** (probe BoW 0.94 / column-aware ordered 0.93 on the two-column body; `docs/probes/s2_0_geometry_alignment.md`) — geometry primary where the S2.1 matcher is confident, fall back to content + structural-path where `geom.present=false`. Re-attach algorithm with an **explicit confidence threshold** defaulting toward **fail-loud on doubt**: unique + above-threshold → bind; else fail loud | §3.4, §3.6; D33, R2; `feedback_existing_path_failures_as_evidence` | S2, **S4.5** | re-binding tier: regenerated stream re-binds stored ids under unchanged geometry; the active mode is recorded in lineage | `BUILD` | `TODO` |
