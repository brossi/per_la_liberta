...
> **Status:** Reviewed and finalized. Awaiting go-ahead to start M0 — nothing has been built yet
> (no branch created; the live edition untouched). The review-pass decisions are incorporated below;
> the audit/response exchange that produced them lives in the project conversation history.
...
Reproduce only **deterministic, no-LLM, no-network** steps against copies of committed inputs and
diff against committed live artifacts. Never run download/ocr/cleanup/translate/multi_translate/refine
for reproduction.
...
- `test_typeset_golden` — typeset on copies of the clean/translation/typography inputs matches
  `output/bilingual.html` in a temp docs/static/page-image workspace with deploy-sync disabled
  (normalize CSS version-hash/timestamps before diffing; supplement with semantic assertions for
  anchors, scan links, font/CSS paths, typography spans, and page-overlay bounds).
- `test_adjudicate_golden` (secondary) — reproduces `adjudication_results.json`.
...
- **M3 — `reconcile`, `typeset` (+ `adjudicate`).** Port with golden tests, reproducing current
  behavior (`validate` still uses `it_combined` + spaCy NER; `adjudicate` still uses
  Zingarelli-only lookup). *Done when:* reconciled/flagged/page maps and `bilingual.html` reproduce
  (normalized). Framework can typeset any book **given** clean + translation + page-mapping inputs;
  it cannot yet take arbitrary raw OCR to a finished bilingual edition.
